#!/bin/bash
# P4_TUMOR full SPARCAL pipeline — steps 1-7 + 4-class matrix (step 8), one job.
#
# Usage (submit from project root: cd /data/maiziezhou_lab/leiy4/snv_calling):
#   sbatch run_slurm/P4_tumor/run_pipeline_P4.sh            # run all steps (1..8)
#   sbatch run_slurm/P4_tumor/run_pipeline_P4.sh 5          # resume from step 5
#   sbatch run_slurm/P4_tumor/run_pipeline_P4.sh 8          # only (re)build matrices
#
...
#SBATCH --output=slurm_output/P4_TUMOR/baseQ0mapQ0/pipeline_P4.out
#SBATCH --error=slurm_output/P4_TUMOR/baseQ0mapQ0/pipeline_P4.err
...
SECTION_ID=1                # P4 dedup ablation: replicate 1 only
DATASET=P4_TUMOR
...
echo "=== P4 pipeline | section ${SECTION_ID} | start step ${START_STEP} | $(date) ==="
...
# 7. spatial filter (clone + CNV integration via CalicoST P4_sec1)
run_step 7 python scripts/6_spatial_filter/run_spatial_snv_filter_enhanced.py \
    ...
    --clone_labels     "${CALICOST_BASE}/P4_sec1/calicost/clone3_rectangle0_w1.0/clone_labels.tsv" \
    --cnv_segments     "${CALICOST_BASE}/P4_sec1/calicost/clone3_rectangle0_w1.0/cnv_seglevel.tsv" \
