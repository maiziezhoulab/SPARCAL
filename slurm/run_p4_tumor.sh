#!/bin/bash
#SBATCH --job-name=sparcal_P4
#SBATCH --output=slurm_output/P4_TUMOR/sparcal_P4-%j.out
#SBATCH --error=slurm_output/P4_TUMOR/sparcal_P4-%j.err
...
for SID in "${SECTIONS[@]}"; do
    ...
    # Somatic: skip germline_filter (steps 3-1 through 3-3), go straight to BAM filter
    # BAM filter uses Beagle-only variants (no classifier output)
    ...
    # Somatic spatial filter (reads CalicoST purity/clone data)
    # TODO: uncomment when 1_somatic_spatial_filter.py is added
...
echo ""; echo "All P4_TUMOR sections complete! $(date)"