...
agreement (0.350 vs 0.363, P = 0.021). The COSMIC comparison changes after extended-MHC
exclusion and is therefore presented as a same-basis external benchmark rather than driver validation. The
...
| **M5** | **Resolved by author decision.** COSMIC is retained as a same-basis external benchmark across classes and callers; it is not presented as variant-level validation or evidence that individual calls are cancer-driving. |
| **M6** | **The MHC finding is correctly identified and should be reflected in reporting.** The extended MHC is exceptionally polymorphic and highly expressed in tumour and immune-infiltrated tissue. Report its exclusion as a separate sensitivity analysis and distinguish somatic HLA LOH from possible mismapping where the data allow. |
