...
def generate_histogram(stats: Dict, output_dir: str, experiment: str, section_id: str, zoom_range: int = 50):
    """Generate a histogram of SNV counts.
    
    Args:
        stats: Dictionary of SNV statistics
        output_dir: Directory to save output files
        experiment: Experiment name
        section_id: Section ID
        zoom_range: Maximum SNV count for the zoomed histogram view
    """
    ...
    plt.title(f"Distribution of SNV Counts - {experiment.upper()} Section {section_id}")
    plt.xlabel("Number of SNVs per spot")
    plt.ylabel("Number of spots")
    plt.grid(True, alpha=0.3)
    
    # Zoom in on the lower counts using the user-specified range
    plt.subplot(2, 1, 2)
    plt.hist(stats["raw_counts"], bins=50, range=(0, zoom_range), edgecolor='black')
    plt.title(f"Distribution of SNV Counts (0-{zoom_range} range)")
    plt.xlabel("Number of SNVs per spot")
    plt.ylabel("Number of spots")
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    parser.add_argument("--zoom-range", type=int, default=1000,
                        help="Maximum SNV count for the zoomed histogram view (default: 50)")
    ...
    # Generate histogram with user-specified zoom range
    generate_histogram(stats, output_dir, args.experiment, args.section_id, args.zoom_range)
...
# python /data/maiziezhou_lab/yuqi/snv_calling/scripts/tools/analyze_snv_stats.py --experiment dlpfc --section-id 151507 --zoom-range 1000

# # For P4_TUMOR section 1:
# python /data/maiziezhou_lab/yuqi/snv_calling/scripts/tools/analyze_snv_stats.py --experiment p4 --section-id 1

# # For P6_TUMOR section 2:
# python /data/maiziezhou_lab/yuqi/snv_calling/scripts/tools/analyze_snv_stats.py --experiment p6 --section-id 2

# # Example with custom zoom range:
# python /data/maiziezhou_lab/yuqi/snv_calling/scripts/tools/analyze_snv_stats.py --experiment dlpfc --section-id 151507 --zoom-range 100