...
DATASET_CONFIGS = {
    ...
    "DCIS": {
        ...
        "output_base": "/data/maiziezhou_lab/leiy4/snv_calling/data/dcis{section_id}",
        # template: {base_path}/DCIS{section_id}/spaceranger_align_DCIS{section_id}_hg38/DCIS{section_id}_output/outs
        "spaceranger_dir_template": "DCIS{section_id}/spaceranger_align_DCIS{section_id}_hg38/DCIS{section_id}_output/outs",
        "spatial_subdir": "spatial",
        "position_file": "tissue_positions.csv",
        "scale_file": "scalefactors_json.json",
        "image_file": "tissue_hires_image.png",
        "has_header": True,      # spaceranger >= 1.3 includes a header row
        "in_tissue_column": 1,    # filter spots where in_tissue == 1
        "snv_vcf_path_template": "/data/maiziezhou_lab/leiy4/snv_calling/data/dcis{section_id}/output_VCFs/spotprofiles/{quality_filter}/vcf_by_spot"
...
class EnhancedSpatialSNVFilter:
    ...
    def setup_paths(self, output_dir=None):
        ...
        # SNV per-barcode VCFs — path convention differs by dataset
        if "snv_vcf_path_template" in cfg:
            self.snv_pos_dir = cfg["snv_vcf_path_template"].format(
            ...
            self.snv_pos_dir = os.path.join(
                output_base,
                self.section_id,
                f"output_VCFs/BAM_filtered/{self.quality_filter}/snv_vcf"
            )
        ...
        if self.dataset == "DLPFC":
            ...
            # {base_path}/DCIS{N}/spaceranger_align_DCIS{N}_hg38/DCIS{N}_output/outs/spatial/
            outs_dir   = os.path.join(cfg["base_path"],
                                      cfg["spaceranger_dir_template"].format(section_id=self.section_id))
            spatial_base = os.path.join(outs_dir, cfg["spatial_subdir"])
            self.positions_file    = os.path.join(spatial_base, cfg["position_file"])
            self.scale_factor_file = os.path.join(spatial_base, cfg["scale_file"])
            self.image_file        = os.path.join(spatial_base, cfg["image_file"])
            self.out_tissue_file   = None   # no missing_barcodes.txt for DCIS; in_tissue column used instead
        else:
            # P4 / P6 — section-specific GSM-named files
...
# REPLACE the whole load_spot_positions method with this:
    def load_spot_positions(self):
        ...
        if self.dataset == "DLPFC":
            # no header; use pixel cols 4,5 (0-based index)
            ...
            for _, row in df.iterrows():
                ...
                if bc in out_tissue_barcodes:
                    continue
                x = float(row[4])  # pxl_row_in_fullres
                y = float(row[5])  # pxl_col_in_fullres
                self.spot_positions[bc] = (x, y)
        elif self.dataset == "DCIS":
            # Standard spaceranger >= 1.3 output — has header row, use in_tissue column
            df = pd.read_csv(self.positions_file, header=0)
            ...
            for _, row in df.iterrows():
                ...
                if bc in out_tissue_barcodes:
                    continue
                x = float(row["pxl_row_in_fullres"])
                y = float(row["pxl_col_in_fullres"])
                self.spot_positions[bc] = (x, y)
        else:
            # P4/P6 tissue_positions_list.csv (no header); use array_x/array_y cols 4,5
            df = pd.read_csv(self.positions_file, header=None)
            for _, row in df.iterrows():
                bc = row[0]
                if bc in out_tissue_barcodes:
                    continue
                x = float(row[4])  # array_x
                y = float(row[5])  # array_y
                self.spot_positions[bc] = (x, y)
    ...
    def load_scale_factors(self) -> Dict:
        """Load scale factors for visualization."""
        try:
            with open(self.scale_factor_file, 'r') as f:
                scale_factors = json.load(f)
            return scale_factors
        except Exception as e:
            logger.warning(f"Could not load scale factors: {e}")
            return {'tissue_hires_scalef': 1.0}
...
# REPLACE the whole build_spatial_graph method with this:
    def build_spatial_graph(self):
        if not self.spot_positions:
            raise ValueError("No spot positions loaded")

        # derive a distance threshold in pixels from scalefactors
        try:
            with open(self.scale_factor_file, 'r') as f:
                sf = json.load(f)
            spot_diameter = float(sf.get("spot_diameter_fullres", 100.0))
        except Exception as e:
            logger.warning(f"Failed to read scale factors: {e}. Using spot_diameter=100.")
            spot_diameter = 100.0

        neighbor_threshold = self.neighbor_distance * spot_diameter  # same semantics as Previous

        barcodes = list(self.spot_positions.keys())
        positions = np.array([self.spot_positions[b] for b in barcodes])

        # kNN first (k=13 like Previous MAX_NEIGHBORS + self), then apply threshold
    ...
    def classify_variants(self, 
                          germline_threshold: float = 0.3,
                          somatic_threshold: float = 0.2):
        """Classify variants as germline, somatic, or ambiguous."""
        ...
        for variant in all_variants:
            # Get race for this variant
            race = self.snv_race.get(variant, "unknown")

            # ── Defined variants (from Beagle/1kG imputation) are germline by definition ──
            # They were identified via population-level imputation and must never appear
            # in somatic_defined or ambiguous_defined regardless of spatial score.
            if race == "defined":
                self.germline_variants.add(variant)
                self.germline_defined.add(variant)
                # Assign a canonical score so the scores file is still populated
                self.variant_scores[variant] = {'germline': 1.0, 'somatic': 0.0}
                continue

            # Check if it's a kept variant (automatically germline)
            if variant in self.kept_variants_pool:
                self.germline_variants.add(variant)
                self.germline_denovo.add(variant)  # kept_variants are denovo that we force germline
                self.variant_scores[variant] = {
                    'germline': 1.0,  # Max score for kept variants
                    'somatic': 0.0
                }
                continue

            # ── Denovo variants — run through spatial scoring classifier ──
            # Check expression thresholds
            meets_germline_expr = self.check_expression_threshold(
                variant, self.min_expression_germline
            )
            meets_somatic_expr = self.check_expression_threshold(
                variant, self.min_expression_somatic
            )

            if not meets_germline_expr and not meets_somatic_expr:
                continue

            # Calculate scores
            germline_score = self.calculate_germline_score(variant)
            somatic_score = self.calculate_somatic_score(variant)

            # Store scores
            self.variant_scores[variant] = {
                'germline': germline_score,
                'somatic': somatic_score
            }

            # Classify denovo variants — only denovo can be somatic or ambiguous
            if germline_score > germline_threshold and somatic_score < somatic_threshold:
                self.germline_variants.add(variant)
                self.germline_denovo.add(variant)
            elif somatic_score > somatic_threshold and germline_score < germline_threshold:
        ...
        logger.info(f"  Somatic (denovo only): {len(self.somatic_variants)} "
                   f"(denovo: {len(self.somatic_denovo)})")
        logger.info(f"  Ambiguous (denovo only): {len(self.ambiguous_variants)} "
                   f"(denovo: {len(self.ambiguous_denovo)})")
        logger.info(f"  Note: all 'defined' variants (from Beagle/1kG) are classified as germline_defined")
    ...
    def visualize_snv_counts(self, 
                           variant_set: Set[str], 
                           variant_type: str,
                           output_file: str):
        """Visualize SNV counts on tissue image."""
        try:
            # Check if image file exists
            if not os.path.exists(self.image_file):
                logger.warning(f"Image file not found: {self.image_file}, skipping visualization")
                return
            ...
            for barcode in self.spot_positions:
                if variant_type == "germline":
                    count = len(self.filtered_spot_snvs_germline.get(barcode, set()))
                elif variant_type == "somatic":
                    count = len(self.filtered_spot_snvs_somatic.get(barcode, set()))
                elif variant_type == "ambiguous":
                    count = len(self.filtered_spot_snvs_ambiguous.get(barcode, set()))
                else:  # combined
                    count = (len(self.filtered_spot_snvs_germline.get(barcode, set())) +
                            len(self.filtered_spot_snvs_somatic.get(barcode, set())) +
                            len(self.filtered_spot_snvs_ambiguous.get(barcode, set())))
            ...
            # Choose colormap
            if variant_type == "germline":
                cmap = LinearSegmentedColormap.from_list('', COLORMAP_GERMLINE)
            elif variant_type == "somatic":
                cmap = LinearSegmentedColormap.from_list('', COLORMAP_SOMATIC)
            else:
                cmap = LinearSegmentedColormap.from_list('', COLORMAP_COMBINED)
            ...
            for barcode, count in count_map.items():
                ...
                if self.dataset == "DLPFC":
                    ...
                    # pxl_col_in_fullres → image x-axis, pxl_row_in_fullres → image y-axis
                    x_scaled = y * scale
                    y_scaled = x * scale
                else:
                    x_scaled = x * scale
            ...
            # Plot
            if x_coords:  # Only plot if there are coordinates
                ...
                cbar.set_label(f'{variant_type.capitalize()} SNV Count')
            
            # Add title
            title = f"{variant_type.capitalize()} Variants - {self.dataset} Section {self.section_id}"
            ...
            stats_text = (
                ...
                f"Max SNVs/spot: {max_count if count_map else 0}\n"
                f"Mean SNVs/spot: {np.mean(list(count_map.values())) if count_map else 0:.2f}"
            )
    ...
    def run_analysis(self):
        ...
        viz_germline  = os.path.join(self.viz_dir, "germline_variants.png")
        viz_somatic   = os.path.join(self.viz_dir, "somatic_variants.png")
        viz_ambiguous = os.path.join(self.viz_dir, "ambiguous_variants.png")
        viz_combined  = os.path.join(self.viz_dir, "all_variants.png")
        
        self.visualize_snv_counts(self.germline_variants, "germline", viz_germline)
        self.visualize_snv_counts(self.somatic_variants, "somatic", viz_somatic)
        self.visualize_snv_counts(self.ambiguous_variants, "ambiguous", viz_ambiguous)
        self.visualize_snv_counts(
            self.germline_variants | self.somatic_variants | self.ambiguous_variants,
            "combined",
            viz_combined
        )
...
Examples:
  ...
  python %(prog)s --dataset dlpfc --section_id 151507 --quality_filter baseQ13mapQ20 \\
    ...
    parser.add_argument('--tumor_purity_file', required=True,
