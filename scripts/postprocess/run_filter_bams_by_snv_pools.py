...
def save_detected_snvs(output_dir: str, result: Dict):
    """
    Save detected SNVs to a text file named after the BAM file.
    ...
    output_file = os.path.join(output_dir, f"{barcode}.txt")
        ...
        with pysam.AlignmentFile(input_bam, "rb") as in_bam:
            ...
            for read in in_bam.fetch(chrom):
                ...
                while j < len(positions) and positions[j] <= read_end:
                    if read_start <= positions[j] <= read_end:
                        filtered_reads.append(read)
                        detected_positions.add(positions[j])
                        break
                    j += 1
    except Exception as e:
...
def filter_bam_by_positions(input_bam: str, output_bam: str, positions_by_chrom: Dict[str, List[int]]) -> Dict:
    """
    ...
    try:
        ...
        detected_snvs = set()  # Set to store detected SNVs (chrom, pos)
        ...
        with pysam.AlignmentFile(input_bam, "rb") as in_bam:
            ...
            # Print some debug info about references in the BAM file
            print(f"Sample of references in {os.path.basename(input_bam)}: {list(bam_references)[:5]}")
            ...
            with pysam.AlignmentFile(temp_output, "wb", header=in_bam.header) as out_bam:
                ...
                for chrom, positions in positions_by_chrom.items():
                    ...
                    # Add detected positions to our set with correct chromosome name
                    for pos in detected_positions:
                        detected_snvs.add((bam_chrom, pos))
...
def filter_bams_parallel(input_bams: List[str], output_dir: str, snvs: Set[SNVInfo], 
    ...
    # Create and ensure the SNV positions directory exists
    snv_txt_dir = os.path.join(os.path.dirname(output_dir), "snv_positions")
    os.makedirs(snv_txt_dir, exist_ok=True)
    print(f"SNV position files will be saved to: {snv_txt_dir}")
    ...
    for snv in snvs:
        ...
        # Also add with 'chr' prefix to handle different reference styles
        if not chrom.startswith('chr'):
            positions_by_chrom[f"chr{chrom}"].append(snv.pos)
    ...
    if len(input_bams) > 10000:
        ...
        for bam in sample_bams:
            ...
            if result.get('detected_snvs'):
                ...
                save_success = save_detected_snvs(snv_txt_dir, result)
                print(f"  Saved SNVs successfully: {save_success}")
                if not save_success:
                    print(f"  Failed to save SNVs to {snv_txt_dir}")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(future_to_bam), 
            ...
            try:
                ...
                # Save detected SNVs to barcode.txt file
                if save_detected_snvs(snv_txt_dir, result):
                    saved_bam_count += 1
    ...
    if len(results) > 0:
        ...
        print(f"SNV position files saved: {saved_bam_count}")
    
    # Verify SNV files were created
    if saved_bam_count > 0:
        snv_files = glob.glob(os.path.join(snv_txt_dir, "*.txt"))
        print(f"Found {len(snv_files)} SNV position files in {snv_txt_dir}")
        if len(snv_files) != saved_bam_count:
            print("Warning: Number of SNV files doesn't match saved_bam_count")
...
class SNVMatrixGenerator:
    ...
    def collect_snvs(self) -> Set[SNVInfo]:
        ...
        for source, path in vcf_files.items():
            ...
            if source == "Beagle":
                ...
                svm_snvs = source_snvs
        
        # Print set overlap statistics for debugging
        if beagle_snvs and svm_snvs:
            overlap = beagle_snvs.intersection(svm_snvs)
            print(f"\nOverlap between Beagle and SVM SNVs: {len(overlap)}")
            print(f"SNVs unique to Beagle: {len(beagle_snvs - svm_snvs)}")
            print(f"SNVs unique to SVM: {len(svm_snvs - beagle_snvs)}")
    ...
    def filter_bams(self, max_workers=30):
        ...
        # Create a directory for SNV text files - make sure it's in the right place
        snv_txt_dir = os.path.join(os.path.dirname(self.filtered_bam_dir), "snv_positions")
        os.makedirs(snv_txt_dir, exist_ok=True)
        print(f"SNV position files will be saved to: {snv_txt_dir}")
        ...
        # Additional verification - check if SNV position files were created
        snv_files = glob.glob(os.path.join(snv_txt_dir, "*.txt"))
        print(f"Found {len(snv_files)} SNV position files after processing")
        ...
        print(f"SNV position files are located in: {snv_txt_dir}")
...
# Usage examples:
# For DLPFC :
# python scripts/postprocess/run_filter_bams_by_snv_pools.py --dataset DLPFC --section-id 151507 --quality-filter baseQ13mapQ20

# For DLPFC (b0m0):
# python scripts/postprocess/run_filter_bams_by_snv_pools.py --dataset DLPFC --section-id 151507 --quality-filter baseQ0mapQ0

# For P4_TUMOR:
# python scripts/postprocess/run_filter_bams_by_snv_pools.py --dataset P4_TUMOR --section-id 1 --quality-filter baseQ0mapQ0

# For P6_TUMOR:
# python scripts/postprocess/run_filter_bams_by_snv_pools.py --dataset P6_TUMOR --section-id 1 --quality-filter baseQ0mapQ0