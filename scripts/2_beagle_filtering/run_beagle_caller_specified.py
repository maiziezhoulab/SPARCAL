...
DATASET_CONFIGS = {
    ...
    "P4_TUMOR": {
        "base_path": "maiziezhou_lab/maiziezhou_lab/Datasets/ST_datasets/STmut_Data/P4_Visium",
...
CALLER_CONFIGS = {
    "mpileup": {
        "path_template": "{output_base}/output_VCFs/mpileup_multi_bam/{quality_filter}/merged_sorted_gt.vcf.gz",
        "requires_quality_filter": True,
        "description": "BCFtools mpileup multi-sample calling",
        "annotation_fields": "INFO/DP,INFO/I16,INFO/QS,INFO/SGB,INFO/RPB,INFO/MQB,INFO/MQSB,INFO/BQB,INFO/MQ0F,FORMAT/GQ,FORMAT/BAF,FORMAT/PL"

    },
    "GATK": {
        "P4_TUMOR": {
            "path": "/lfs/archer.accre.vu/maiziezhou_lab/maiziezhou_lab/Datasets/ST_datasets/STmut_Data/P4_cSCC_WES/P4_cSCC_WES_gatk_snp_chr1_22.vcf.gz",
            "description": "GATK HaplotypeCaller germline SNPs"
        },
        "requires_quality_filter": False,
        "annotation_fields": "INFO/DP,FORMAT/GQ,FORMAT/PL,FORMAT/AD"
    },
    "Mutect2": {
        "P4_TUMOR": {
            "path": "/lfs/archer.accre.vu/maiziezhou_lab/maiziezhou_lab/Datasets/ST_datasets/STmut_Data/P4_Somatic_Mutect2/P4_somatic_snp_chr1_22.vcf.gz",
            "description": "Mutect2 somatic SNPs"
        },
        "requires_quality_filter": False,
        "annotation_fields": "INFO/DP,FORMAT/AD,FORMAT/AF"
    },
    "Monopogen": {
        "P4_TUMOR": {
            "path": "/lfs/archer.accre.vu/maiziezhou_lab/maiziezhou_lab/CanLuo/ST_SNV/Monopogen/P4_rep{section_id}/out/germline/merged.vcf.gz",
            "description": "Monopogen germline variants"
        },
        "requires_quality_filter": False,
        "annotation_fields": None  # Monopogen has no INFO fields to merge
    }
}

class BeaglePipeline:
    def __init__(self, dataset_name: str, quality_filter: str = "baseQ0mapQ0", 
                 section_id: str = None, caller: str = "mpileup"):
        self.dataset_name = dataset_name
        self.quality_filter = quality_filter
        self.section_id = section_id
        self.caller = caller
        self.validate_dataset_config()
        self.validate_caller_config()
        self.setup_paths()
    ...
    def validate_caller_config(self):
        """Validate caller configuration."""
        if self.caller not in CALLER_CONFIGS:
            raise ValueError(f"Unknown caller: {self.caller}. "
                           f"Valid callers are: {list(CALLER_CONFIGS.keys())}")
        
        caller_config = CALLER_CONFIGS[self.caller]
        
        # Check if caller requires quality filter
        if caller_config.get("requires_quality_filter", False):
            if not self.quality_filter:
                raise ValueError(f"Caller {self.caller} requires a quality_filter parameter")
        
        # For non-mpileup callers, check if dataset-specific path exists
        if self.caller != "mpileup":
            if self.dataset_name not in caller_config:
                raise ValueError(f"Caller {self.caller} does not have configuration for dataset {self.dataset_name}. "
                               f"Available datasets for {self.caller}: {[k for k in caller_config.keys() if k != 'requires_quality_filter']}")
    
    def setup_paths(self):
        """Setup paths based on dataset configuration and caller."""
        dataset_config = DATASET_CONFIGS[self.dataset_name]
        ...
        # Setup output directories with quality filter and caller-specific naming
        # For mpileup, keep the original quality_filter subdirectory structure
        # For other callers, use caller name as subdirectory
        if self.caller == "mpileup":
            filter_subdir = self.quality_filter
        else:
            filter_subdir = self.caller
    ...
    def get_input_vcf_path(self) -> str:
        """Get the input VCF path based on caller configuration."""
        caller_config = CALLER_CONFIGS[self.caller]
        
        if self.caller == "mpileup":
            # Use template-based path for mpileup
            path_template = caller_config["path_template"]
            input_vcf = path_template.format(
                output_base=self.output_base,
                quality_filter=self.quality_filter
            )
        else:
            # Use direct path for other callers
            dataset_caller_config = caller_config[self.dataset_name]
            input_vcf = dataset_caller_config["path"]
            
            # Handle section_id placeholder if present
            if "{section_id}" in input_vcf:
                if not self.section_id:
                    raise ValueError(f"Caller {self.caller} requires section_id for dataset {self.dataset_name}")
                input_vcf = input_vcf.format(section_id=self.section_id)
    ...
    def merge_vcf_fields(self, original_vcf: str, beagle_vcf: str, output_vcf: str):
        ...
        # Get annotation fields for this caller
        caller_config = CALLER_CONFIGS[self.caller]
        annotation_fields = caller_config.get("annotation_fields")
        
        # If no annotation fields, just copy the Beagle output
        if annotation_fields is None:
            print(f"  Caller {self.caller} has no INFO/FORMAT fields to merge, using Beagle output directly")
            shutil.copy2(beagle_vcf, output_vcf)
            # ... handle indexing ...
            return
        
        # Otherwise, proceed with caller-specific annotation fields
        cmd = [
            'bcftools', 'annotate',
            '-a', original_vcf,
            '-c', annotation_fields,  # Caller-specific fields
            '-O', 'z',
            '-o', output_vcf,
            beagle_vcf
        ]
    ...
    def run_beagle_command(self, input_vcf: str, output_prefix: str, 
        ...
        cmd = [
            ...
            f"gt={input_vcf}",  # changed from GL to GT
    ...
    def process_chromosome(self, chromosome: str, params: Dict) -> Dict:
        ...
        input_vcf = self.get_input_vcf_path()
    ...
    def run_pipeline(self, custom_params: Optional[Dict] = None) -> pd.DataFrame:
        ...
        # Get input VCF path based on caller
        input_vcf = self.get_input_vcf_path()
        print(f"Using input VCF from {self.caller}: {input_vcf}")
...
def main():
    ...
    parser.add_argument("--caller", default="mpileup", 
                      choices=list(CALLER_CONFIGS.keys()),
                      help="Variant caller used to generate input VCF (default: mpileup)")
    ...
    parser.add_argument("--quality-filter", default="baseQ0mapQ0",
                      help="Quality filter directory (default: baseQ0mapQ0, only used for mpileup caller)")
    ...
    print(f"Variant caller: {args.caller}")
    print(f"Using chromosome format: {pipeline.chr_prefix}N")
    if args.caller == "mpileup":
...
# 1. Run with default mpileup caller on P4_tumor, baseQ0mapQ0
# python scripts/filtering/run_beagle.py --dataset P4_TUMOR --section_id 1 --quality-filter baseQ0mapQ0 --caller mpileup

# 2. Run with GATK caller on P4_tumor section 1
# python scripts/filtering/run_beagle.py --dataset P4_TUMOR --section_id 1 --caller GATK

# 3. Run with Mutect2 caller on P4_tumor section 1
# python scripts/filtering/run_beagle.py --dataset P4_TUMOR --section_id 1 --caller Mutect2

# 4. Run with Monopogen caller on P4_tumor section 1
# python scripts/filtering/run_beagle.py --dataset P4_TUMOR --section_id 1 --caller Monopogen

# 5. Run beagle on DLPFC 151507 with mpileup
# python scripts/filtering/run_beagle.py --dataset DLPFC --quality-filter baseQ0mapQ0 --section_id 151507 --caller mpileup